...
BIN = r'C:\Windows\System32\user32.dll'
...
ref = parse(lines)
ref = [(a, b, t) for a, b, t in ref if '(bad)' not in t]
start = ref[0][0]
end = ref[-1][0] + len(ref[-1][1])
rr = subprocess.run([GHR, 'spec', SPEC, BIN, 'disasm', hex(start), str(end - start), '--resume'],
                    capture_output=True, text=True)
gaps = [int(l.split()[2], 16) for l in rr.stdout.splitlines() if l.startswith('; GAP')]
seen = set()
for a, b, txt in ref:
    if a in set(gaps):
        key = ' '.join(b)
        if key in seen:
            continue
        seen.add(key)
        print('%#x: %-36s %s' % (a, ' '.join(b), txt.split('\t')[-1].strip()))
