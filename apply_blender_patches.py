...
def main():
    ...
    # 10. external_140479490: Blender-internal wide->narrow argv conversion
    #     (wmain argv path).  Plain stub returned 0, so every converted argv
    #     entry became NULL and the init strcmp(0, "...") crashed in
    #     ucrtbase.  Implement with host WideCharToMultiByte.
    stub_path2 = src_dir + '/external_stubs.cpp'
    if os.path.exists(stub_path2):
        src = load(stub_path2)
        old = ('external_140479490(std::uint64_t a0, std::uint64_t a1, std::uint64_t a2, '
               'std::uint64_t a3, std::uint64_t a4, std::uint64_t a5, std::uint64_t a6, '
               'std::uint64_t a7) {\n'
               '    (void)a0; (void)a1; (void)a2; (void)a3; (void)a4; (void)a5; '
               '(void)a6; (void)a7;\n'
               '    return 0;\n}')
        if old in src:
            new = ('external_140479490(std::uint64_t a0, std::uint64_t a1, std::uint64_t a2, '
                   'std::uint64_t a3, std::uint64_t a4, std::uint64_t a5, std::uint64_t a6, '
                   'std::uint64_t a7) {\n'
            ...
            save(stub_path2, src.replace(old, new))
            print('external_140479490 argv WideCharToMultiByte applied')
        else:
            print('external_140479490 stub not plain (already patched or recovered)')
